...
int parse_gps_conf() {
    ...
    fclose(file);
    LOGx("%s() is called: GPS1_XTRA_AUTO_DOWNLOAD_ENABLED = %d", __FUNCTION__, XTRA_AUTO_DOWNLOAD_ENABLED);
    LOGx("%s() is called: GPS1_XTRA_DOWNLOAD_INTERVAL = %d", __FUNCTION__, XTRA_DOWNLOAD_INTERVAL);
    LOGx("%s() is called: GPS1_CLEANUP_ENABLED = %d", __FUNCTION__, CLEANUP_ENABLED);
    LOGx("%s() is called: GPS1_SESSION_TIMEOUT = %d", __FUNCTION__, SESSION_TIMEOUT);
    LOGx("%s() is called: GPS1_MEASUREMENT_PRECISION = %d", __FUNCTION__, MEASUREMENT_PRECISION);
    return 0;
}
...
int init_leo() 
{
    ...
    if (!CHECKED[0]) {
        if (use_nmea)
            LOGx("%s() is called: %s version", __FUNCTION__, "NMEA");
        else
            LOGx("%s() is called: %s version", __FUNCTION__, "RPC");
